@extends('layouts.app')
@section('content')
    <form action="/material" method="POST" class="form-horizontal">
        <div class="panel-body">
        @csrf
        <!-- Task Name -->
            <div class="form-group">
                <table>
                    <tr>
                        <td><label>재료명</label></td>
                        <td><input type="text" name="name" id="name" value="{{old('name')}}" class="form-control"></td>
                    </tr>
                    <tr>
                        <td><label>원산지</label></td>
                        <td><input type="text" name="origin" id="origin" value="{{old('origin')}}" class="form-control"></td>
                    </tr>
                    <tr>
                        <td><label>비고</label></td>
                        <td><input type="text" name="bigo" id="bigo" value="{{old('bigo')}}" class="form-control"></td>
                    </tr>
                </table>
            </div>
        </div>
        <table>
            <tr>
                <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-6">
                        <button type="submit" class="btn btn-default">
                            <i class="fa fa-plus"></i> 저장
                        </button>
                    </div>
                </div>
            </tr>
        </table>
    </form>
    <!-- Current Tasks -->

@endsection